...
const DeletionAlert = ({ data, isOpen, onClose }) => {
  ...
  const deleteTask = async () => {
    ...
    try {
      const res = await axios.post(`${url}/api/actions/delete`, { id: data._id }).then(res => res.data);
    ...
    <AlertDialog isOpen={isOpen} leastDestructiveRef={cancelRef} onClose={onClose}>
      <AlertDialogOverlay>
        <AlertDialogContent>
          ...
          <AlertDialogBody>Are you sure you want to permanently delete this task?</AlertDialogBody>

          <AlertDialogFooter>
            {isErr && (
              <Text m={5} color="red">
                Error deleting this task
              </Text>
            )}
